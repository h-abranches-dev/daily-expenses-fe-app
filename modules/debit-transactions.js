...
function setRowToEdit(id, td, tr, tc, a) {
    rowToEdit.id = id;
...
async function drawDebitTransactionsTable(entity, type) {
    ...
    document.getElementById('debitTransactions').innerHTML = t;
...
async function renderTable() {
    ...
    if (transactions != undefined && transactions != null && transactions.length > 0) {
        transactions.forEach(t => {
            const tObj = new DebitTransaction(t.id, t.transaction_date, t.transaction, t.categories, t.amount);
    ...
    rows += `<td><input type="text" id="newTransaction" value="new transaction"></td>`;
...
function clickOnEditButtonEventListener() {
    clickOnEditButtonEvent(this.id, transactionsIds);
}
...
export { drawDebitTransactionsTable, DebitTransaction };
